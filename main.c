...
int input[2] = {0, 0};
int start = 1; //Init as 0 and set to 1 when both inputs are inputted
int displayFlag = 0;
int motor1Counter = 0;
int motor2Counter = 0;
int inputCounter = 0;
...
void main(void)
{
    ...
    {
        if (naturalDeath == 1 && inputCounter == 2 && onceFlag == 1) {
            killSwitch = 0;
            naturalDeath = 0;
            motor1Counter = 0;
            motor2Counter = 0;
        ...
        if ((GPIO_getInputPinValue(SW1_PORT, SW1_PIN) == 0) & buttonState == 0) {
            buttonState = 1;
        }
        if ((GPIO_getInputPinValue(SW1_PORT, SW1_PIN) == 1) & buttonState == 1) {
            killSwitch = 1;
            displayFlag = 1;
            naturalDeath = 0;
            buttonState = 0;
        }
        if (killSwitch == 0 && (motor1Counter < input[0] || motor2Counter < input[1])) {
            runMotor();
        }

        if (motor1Counter >= input[0] && motor2Counter >= input[1] && killSwitch == 0 && inputCounter == 2) {
            killSwitch = 1;
            naturalDeath = 1;
            displayFlag = 1;
        }
        ...
        if (ADCState == 0 && killSwitch == 0)
        {
            if (ADCResult > 0) {
                checkADC = 1;
            }
            if (ADCResult < 300 && checkADC == 1) {
                killSwitch = 1;
                naturalDeath = 0;
                displayFlag = 1;
            }
...
{
    //Init Motor 1
    P1DIR |= BIT7;//IN1 for P1.7
    P1DIR |= BIT6;//IN2 for P1.6
    P5DIR |= BIT0;//IN3
    P5DIR |= BIT2;//IN4
...
void EUSCIA0_ISR(void)
{
    ...
    if (RxStatus)
    {
        int in = EUSCI_A_UART_receiveData(EUSCI_A0_BASE);
        in = in - 48;
        if (inputCounter == 0) {
            if (in == -35) {
            ...
            input[0] = input[0] * 10 + in;
            EUSCI_A_UART_transmitData(EUSCI_A0_BASE, in + 48);
